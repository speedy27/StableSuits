 ...
 * iOS's `.contentTransition(.numericText())`. Only the digits that actually
 * change blur as they move; unchanged digits never animate.
 * Single file, zero runtime dependencies (React peer only).
 */
...
@keyframes an-resolve {
  from { opacity: 0; filter: blur(var(--an-blur, 14px)); }
  to { opacity: 1; filter: blur(0); }
}
@keyframes an-dissolve {
  from { opacity: 1; filter: blur(0); }
  to { opacity: 0; filter: blur(var(--an-blur, 14px)); }
}
